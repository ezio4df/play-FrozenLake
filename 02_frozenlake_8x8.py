...
def train_dqn(agent, env, state_size, action_size, episodes=2000, max_steps=100):
    ...
    for episode in range(episodes):
        ...
        for t in range(max_steps):
            ...
            # observation: when using above, the agent tends to fall in the holes -> quick game over -> less punishment
...
def render_to_opencv_window(env, window_name="FrozenLake Render", delay=50):
    frame = env.render()
    if frame is not None:
        frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
        frame_bgr = cv2.resize(frame_bgr, (400, 400), interpolation=cv2.INTER_NEAREST)
        cv2.imshow(window_name, frame_bgr)
        cv2.waitKey(delay)  # delay in ms. 50ms ~ 20 FPS
